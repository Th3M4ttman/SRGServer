...
def add_text(im,text,W,H,bordercolor=(0,0,0,255),so=[False,False,False],ro=[False,False,False],p1="They", p2="Them"):
	...
	draw.rectangle([x1, y1, x2, y2], width=int(W/50), outline=tuple(int(ti/1.5) for ti in bordercolor))
	
	draw.rectangle([x1, y1, x2, y2], width=int(W/100), outline=bordercolor)
	
	#draw top left corner
	
	draw.rectangle([0, 0, int(W/5), int(H/5)], width=int(W/50), outline=tuple(int(ti/1.5) for ti in bordercolor),fill=tuple(int(ti/1.5) for ti in bordercolor))
	
	draw.rectangle([0, 0, int(W/5), int(H/5)], width=int(W/100), outline=bordercolor)
	
	#draw top right corner
	
	draw.rectangle([int(W-(W/5)), 0, W, int(H/5)], width=int(W/50), outline=tuple(int(ti/1.5) for ti in bordercolor),fill=tuple(int(ti/1.5) for ti in bordercolor))
	
	draw.rectangle([int(W-(W/5)), 0, W, int(H/5)], width=int(W/100), outline=bordercolor)
	
		#draw bottom left corner
	
	draw.rectangle([0, int(H-(H/5)),int(W/5),H], width=int(W/50), outline=tuple(int(ti/1.5) for ti in bordercolor),fill=tuple(int(ti/1.5) for ti in bordercolor))
	
	draw.rectangle([0, int(H-(H/5)),int(W/5),H], width=int(W/100), outline=bordercolor)
		
	#draw bottom right corner
	
	draw.rectangle([int(W-(W/5)), int(H-(H/5)),W,H], width=int(W/50), outline=tuple(int(ti/1.5) for ti in bordercolor),fill=tuple(int(ti/1.5) for ti in bordercolor))
...
def create(srg=[None,False,False],ps="They",pt="Them",cr=255,cg=255,cb=255,res=500,F=None,debug=False):
	...
	if ul not in["y","Y"]:
		if F==None:
			F=pyip.inputStr("Filename: ")+".png"
		else:
			if debug: print(temp.name)
			F=name+".png"
		img.save(F)
		if debug: print(F)
		print("Saved SRG "+str(srg[0])+" to "+str(F))
		temp.close()
		os.remove(temp.name)
		if debug: print('deleted file')
		return srg[0]
	else:
